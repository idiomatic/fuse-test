// Serve a content-encrypted FUSE filesystem.
...
import (
	...
	"github.com/kevinburke/nacl/stream"
)
...
func (f *File) Read(ctx context.Context, req *fuse.ReadRequest, resp *fuse.ReadResponse) error {
	...
	blockOffset := req.Offset / blocksize
	offset := req.Offset - (blockOffset * blocksize)

	cipherSpan := f.content[blockOffset*blocksize:][:offset+req.Size]
	clearSpan := make([]byte, len(cipherSpan))

	for {
		var nonce bytes.Buffer
		nonce.Grow(nacl.NonceSize)
		binary.Write(nonce, struct {
			blockOffset uint64
			inode       uint64
		}{blockOffset, f.Inode})

		cipherBlock := stream.XOR(cipherText[:blocksize], nacl.Nonce(&nonce), key)
		clearSpan = append(clearSpan, cipherBlock)
		blockOffset++
		cipherSpan = cipherText[blocksize:]
	}

	data := clearSpan[offset : offset+req.Size]
	resp.Data = data
	return nil
}
